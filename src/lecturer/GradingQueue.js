...
function GradingQueue({ onNavigate }) {
  ...
  const fetchSubmissions = useCallback(async () => {
    ...
    // Step 1: Get assessments this lecturer created
    const { data: myAsms, error: asmErr } = await supabase
      .from("assessments")
      .select("id, title, questions(marks)")
      .eq("created_by", user.id);

    if (asmErr) {
      console.error("Assessment fetch error:", asmErr);
      setLoading(false);
      return;
    }
    ...
    // Build lookup: assessmentId -> { title, totalMarks }
    const asmLookup = {};
    myAsms.forEach((a) => {
      asmLookup[a.id] = {
        title: a.title,
        totalMarks: (a.questions ?? []).reduce(
          (sum, q) => sum + (q.marks ?? 0),
          0,
        ),
      };
    ...
    // Step 2: Fetch submissions WITHOUT profile join
    const { data: subs, error: subErr } = await supabase
      .from("submissions")
      .select(
        `
        id, status, submitted_at, student_id, assessment_id,
    ...
    const studentIds = [
      ...new Set(subs.map((s) => s.student_id).filter(Boolean)),
    ];
    ...
    const mapped = subs.map((s) => {
      const profile = profileLookup[s.student_id];
      const name =
        profile?.full_name ||
        profile?.email ||
        `Student (${s.student_id?.slice(0, 8) ?? "?"})`;
      const asmInfo = asmLookup[s.assessment_id] ?? {};
      const totalMarks = asmInfo.totalMarks ?? 0;
      const awarded = (s.answers ?? []).reduce(
                ...
                {total > 0 && (
                  <span className="gq-progress-inline">
                    <span
                      ...
                      style={{
                        width: `${Math.round((graded / total) * 100)}%`,
                      }}
                    />
                  </span>
                )}
      ...
      <div className="gq-table-card">
        {loading ? (
          <table className="gq-table">
            <tbody>
              <tr>
                <td colSpan={8} className="gq-empty">
                  Loading submissions…
                </td>
              </tr>
            </tbody>
          ...
          <div className="gq-empty-state">
            ...
            <p className="gq-empty-desc">
              {submissions.length === 0
                ? "Once students submit assessments they will appear here."
                  ...
                  >
                    ...
                    <td>
                      <div className="gq-student">
                        ...
                        <span className="gq-student-name">
                          {sub.studentName}
                        </span>
                      </div>
                    </td>
                    ...
                    <td>
                      <span
                        className={`gq-status-chip ${statusClass(sub.status)}`}
                      >
                        {sub.status}
                      </span>
                    </td>
