...
function NotificationsPanel({
  ...
  const handleClick = (n) => {
    markRead(n.id);
    if (n.href?.startsWith("results/")) onNavigate("results");
    else if (n.href) onNavigate(n.href.replace("/", "-"));
  };
  return (
    <div className="dash-card">
      <div className="dash-card-header">
        ...
        {notifications.length > 0 && (
          <button className="dash-card-action" onClick={clearAll}>
            Clear all
          </button>
        )}
      </div>
      ...
      {unreadCount > 0 && (
        <div className="dash-notif-footer">
          <button onClick={markAllRead} className="dash-card-action">
            Mark all as read
          </button>
        </div>
      )}
  ...
  const fetchData = useCallback(async () => {
    ...
    if (graded.length > 0) {
      const totals = graded
        ...
        .filter(Boolean);
      if (totals.length)
        avgScore = Math.round(totals.reduce((a, b) => a + b) / totals.length);
    }

    setStats({
      available,
      submitted: subList.length,
  ...
  const QA = [
    {
      ...
      desc: "Start an available test",
      page: "student-assessments",
      color: "#e0e7ff",
    },
    {
      icon: <FileText size={22} />,
      label: "Past Results",
      desc: "View graded submissions",
      page: "results",
      color: "#dcfce7",
    },
    {
          ...
          <div className="dash-card" style={{ gridColumn: "span 2" }}>
            ...
            {loading ? (
              ...
              recent.map((r) => (
                <div
                  key={r.id}
                  className="dash-queue-item stu-clickable"
