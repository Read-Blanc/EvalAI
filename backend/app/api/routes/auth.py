...
def signup(user_data: UserSignup, db: Session = Depends(get_db)):
    ...
    **Requirements:**
    ...
    - Valid role: admin, lecturer, or student
    ...
    **Returns:**
    ...
    existing_user = auth_crud.get_user_by_username(db, user_data.username)
    if existing_user:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
def login(user_data: UserLogin, db: Session = Depends(get_db)):
    ...
    **Returns:**
    ...
    """
    # Authenticate user
    user = auth_crud.authenticate_user(db, user_data.username, user_data.password)
